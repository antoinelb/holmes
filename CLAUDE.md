...
- `holmes package` / `holmes p` — zip the 3445 products (45 data products + the 3400-tile basemap pyramid) into `data-YYYY-MM-DD.zip` (`download.package.build_archive`; `--output PATH` optional). Fails listing every missing product.
...
**Neither the wheel nor the repo ships data products** (the repo does keep the MELCC station CSVs, which are inputs, not products). The single distribution channel is the dated zip (`data-YYYY-MM-DD.zip`, ~240 MB since the 2026-08-16 float32 cast of the projections; products and map tiles only — no rasters/DEM/shapefiles/cell caches) on the rolling `data` release, produced nightly by `.github/workflows/data.yml` (07:00 UTC; gated on the `DATA_REFRESH_ENABLED` repo variable, manual `workflow_dispatch` always allowed; CDS credentials from the `CDSAPI_KEY` secret, the Carto basemap key from `CARTO_KEY`; restores the previous zip, runs `holmes download` incrementally, uploads the new zip **then** deletes every other asset). The server consumes it via the startup sync and fetches nothing else — the map tiles ship in the archive too, so the app is fully offline after the sync.
...
- `tests/e2e/` boots a real uvicorn server and needs a warmed `data/` directory (CI downloads the newest `data-*.zip` from the `data` release, unzips it, and writes `data/archive_date.txt` so the startup sync no-ops; locally, run the app once — the sync fetches the same zip, map tiles included).
