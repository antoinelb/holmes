"""Build layer for the projection products.

Copy of the PAVICS build in `holmes.data.projection`, adapted for the
sync `holmes download` orchestrator: skip-if-exists products, staged
writes, task-style printing, and no release-asset fallback (the build
layer only ever talks to the true sources). The original stays in place
until the final cleanup pass.
"""
...
# geometry is imported as a module so tests can patch its helpers and see
# the patched versions used from here
...
def build_projection_data(
    ...
    """Build the per-station projection products from PAVICS.

    One product per station id in `stations`, at
    `raw/projection/{id}.ipc`: daily projected forcing averaged over the
    watershed, per ensemble ("ClimEx" or "ESPO-G6-R2"), scenario and
    member, from 2020-01-01 to 2099-12-30 on a noleap calendar.

    Skip-if-exists: nothing runs when every product is already cached;
    `force` rebuilds them all, ignoring the per-member resume caches.
    """
    if stations.height == 0:
        raise ValueError("No stations to build projection data for.")
    ...
    # the fetches are async for concurrency; the orchestrator is sync
    asyncio.run(
        _build_products(
            stations.filter(pl.col("id").is_in(missing)), rebuild=force
        )
    )
...
async def _build_products(stations: pl.DataFrame, *, rebuild: bool) -> None:
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        with progress_task(
            "Downloading projection members...",
            "Downloaded projection members.",
            total=len(member_tasks),
        ) as progress:
            try:
                for pending in asyncio.as_completed(member_tasks):
                    await pending
                    progress.increment()
            except BaseException:
                # a failed member aborts the whole build (partial data would
                # surface as silent gaps the models reject); reap the rest
                # so nothing keeps hitting the server
                for pending in member_tasks:
                    pending.cancel()
                await asyncio.gather(*member_tasks, return_exceptions=True)
                raise

    with progress_task(
        "Assembling projection products...",
        "Assembled projection products.",
        total=stations.height,
    ) as progress:
        for id_ in stations["id"]:
            data = pl.concat(
                [
                    ...
                    # the ensemble is task metadata, stamped at assembly so
                    # the per-member caches predating the column stay valid
...
def _station_weights(
    ...
    """
    polygons = geometry.to_geopandas(stations).to_crs(grid.crs)
...
async def _read_member(
    ...
    *,
    ...
    for id_, (cells, coverage) in weights.items():
        ...
        _write_ipc(data, _member_path(id_, task.scenario, task.member))
...
def _compute_box_weights(
    ...
    return geometry.compute_coverage_weights(polygons, template)
