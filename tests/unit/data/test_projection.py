...
class TestHasProjectionData:
    def test_missing_product(self, stations_df):
        assert not projection.has_projection_data(stations_df)

    def test_all_products_present(self, tmp_data_dir, stations_df):
        for id_ in stations_df["id"]:
            path = projection._product_path(id_)
            path.parent.mkdir(exist_ok=True, parents=True)
            pl.DataFrame({"id": [id_]}).write_ipc(path)
        assert projection.has_projection_data(stations_df)


class TestReadProjectionData:
    async def test_empty_stations_raises(self, stations_df):
        with pytest.raises(ValueError, match="No stations"):
            await projection.read_projection_data(stations_df.head(0))
    ...
    async def test_cached_products_concat_sorted(
        self, tmp_data_dir, stations_df
    ):
        for id_ in stations_df["id"]:
            self.write_product(id_, "r1")
        data = await projection.read_projection_data(stations_df)
        assert data["id"].to_list() == sorted(stations_df["id"].to_list())

    async def test_release_asset_fills_missing(
        self, tmp_data_dir, monkeypatch, stations_df
    ):
        def fake_asset(asset, path):
            self.write_product(asset.removesuffix(".ipc"), "r1")
            return True

        monkeypatch.setattr(projection, "download_release_asset", fake_asset)
        build = AsyncMock()
        monkeypatch.setattr(projection, "_build_products", build)
        data = await projection.read_projection_data(stations_df)
        build.assert_not_awaited()
        assert data.height == stations_df.height

    async def test_builds_when_assets_unavailable(
        self, tmp_data_dir, monkeypatch, stations_df
    ):
        self.write_product("061004", "r1")

        async def fake_build(stations, *, rebuild):
            assert stations["id"].to_list() == ["061020"]
            assert not rebuild
            self.write_product("061020", "r1")

        monkeypatch.setattr(
            projection, "download_release_asset", lambda asset, path: False
        )
        monkeypatch.setattr(projection, "_build_products", fake_build)
        data = await projection.read_projection_data(stations_df)
        assert sorted(data["id"].to_list()) == ["061004", "061020"]

    async def test_rebuild_skips_assets(
        self, tmp_data_dir, monkeypatch, stations_df
    ):
        asset = MagicMock(side_effect=AssertionError("must not download"))
        monkeypatch.setattr(projection, "download_release_asset", asset)
        ...
        monkeypatch.setattr(projection, "_build_products", fake_build)
        data = await projection.read_projection_data(stations_df, rebuild=True)
        assert data.height == stations_df.height
...
class TestBuildProducts:
    ...
    async def test_assembles_products(
        ...
        for id_ in stations_df["id"]:
            product = pl.read_ipc(projection._product_path(id_))
...
class TestReadMember:
    ...
    async def test_cached_member_returns_early(
    ...
    async def test_climex_member_uses_tas(
        ...
        data = pl.read_ipc(projection._member_path("061004", "rcp8.5", "r1"))
        assert data["temperature"].to_list() == pytest.approx(
            [1.0] * 3, abs=1e-4
        )
